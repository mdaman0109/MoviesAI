    ...
    <div className="bg-black py-4 px-8">
      ...
      <div className="">
        <div className="w-12/12 py-2">
          <div className="flex flex-row overflow-x-scroll scroll-container gap-5">
            {castInfo.map((cast) =>
              cast?.profile_path ? (
                <Link to={"/castmovie/"+cast?.id} onClick={handleCastClick} state={{ actorName: cast?.name }}><div
                  key={cast.id}
                  className="flex justify-between items-center hover:border-2  hover:border-red-700 rounded-lg  flex-col bg-zinc-700 max-h-44 md:max-h-64"
                >
